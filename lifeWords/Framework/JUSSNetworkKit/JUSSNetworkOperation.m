...
#ifdef __OBJC_GC__
#error JUSSNetworkKit does not support Objective-C Garbage Collection
#endif

#if ! __has_feature(objc_arc)
#error JUSSNetworkKit is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

@interface JUSSNetworkOperation (/*Private Methods*/)
...
@property (nonatomic, assign) JUSSNetworkOperationState state;
...
@implementation JUSSNetworkOperation
...
-(BOOL) isEqual:(id)object {
  ...
  if([self.request.HTTPMethod isEqualToString:@"GET"] || [self.request.HTTPMethod isEqualToString:@"HEAD"]) {
    ...
    JUSSNetworkOperation *anotherObject = (JUSSNetworkOperation*) object;
...
-(JUSSNetworkOperationState) state {
  
  return _state;
}

-(void) setState:(JUSSNetworkOperationState)newState {
  
  switch (newState) {
    case JUSSNetworkOperationStateReady:
      [self willChangeValueForKey:@"isReady"];
      break;
    case JUSSNetworkOperationStateExecuting:
      [self willChangeValueForKey:@"isReady"];
      [self willChangeValueForKey:@"isExecuting"];
      break;
    case JUSSNetworkOperationStateFinished:
      [self willChangeValueForKey:@"isExecuting"];
...
- (void)encodeWithCoder:(NSCoder *)encoder 
{
  ...
  self.state = JUSSNetworkOperationStateReady;
...
{
  JUSSNetworkOperation *theCopy = [[[self class] allocWithZone:zone] init];  // use designated initializer
...
-(void) updateHandlersFromOperation:(JUSSNetworkOperation*) operation {
...
{	
  if((self = [super init])) {
    ...
    self.request = [NSMutableURLRequest requestWithURL:finalURL                                                           
                                       ...
                                       timeoutInterval:kJUSSNetworkKitRequestTimeOutInSeconds];
...
/*
 Printing a JUSSNetworkOperation object is printed in curl syntax
 */
...
- (void) start
{
  ...
  if(!self.isCancelled) {
    ...
    self.state = JUSSNetworkOperationStateExecuting;
  }
  else {
    self.state = JUSSNetworkOperationStateFinished;
...
- (BOOL)isReady {
  ...
  return (self.state == JUSSNetworkOperationStateReady);
}

- (BOOL)isFinished 
{
  return (self.state == JUSSNetworkOperationStateFinished);
}

- (BOOL)isExecuting {
  
  return (self.state == JUSSNetworkOperationStateExecuting);
}
...
-(void) cancel {
  ...
  @synchronized(self) {
    ...
    if(self.state == JUSSNetworkOperationStateExecuting)
      self.state = JUSSNetworkOperationStateFinished; // This notifies the queue and removes the operation.
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
  ...
  // if you attach a stream to the operation, JUSSNetworkKit will not cache the response.
  ...
  if([self.request.HTTPMethod isEqualToString:@"GET"] && [self.downloadStreams count] == 0) {
    ...
    if([contentType rangeOfString:@"image"].location != NSNotFound) {
      ...
      if(!eTag && !lastModified)
        expiresOnDate = [[NSDate date] dateByAddingTimeInterval:kJUSSNetworkKitDefaultImageCacheDuration];
      else    
        expiresOnDate = [[NSDate date] dateByAddingTimeInterval:kJUSSNetworkKitDefaultImageHeadRequestDuration];
    }
    ...
    for(NSString *substring in cacheControlEntities) {
      ...
      if([substring rangeOfString:@"no-cache"].location != NSNotFound) {
        ...
        expiresOnDate = [[NSDate date] dateByAddingTimeInterval:kJUSSNetworkKitDefaultCacheDuration];
      }
    }
