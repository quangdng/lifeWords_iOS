...
#define kFreezableOperationExtension @"JUSSNetworkkitfrozenoperation"
...
@interface JUSSNetworkEngine (/*Private Methods*/)
...
@implementation JUSSNetworkEngine
...
// no matter how many instances of JUSSNetworkEngine is created, there is one and only one network queue
...
- (id) initWithHostName:(NSString*) hostName apiPath:(NSString*) apiPath customHeaderFields:(NSDictionary*) headers {
  ...
  if((self = [super init])) {        
    ...
    self.customOperationSubclass = [JUSSNetworkOperation class];
  }
                         ...
                         change:(NSDictionary *)change context:(void *)context
{
  if (object == _sharedNetworkQueue && [keyPath isEqualToString:@"operationCount"]) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kJUSSNetworkEngineOperationCountChanged 
...
-(void) freezeOperations {
  ...
  for(JUSSNetworkOperation *operation in _sharedNetworkQueue.operations) {
...
-(void) checkAndRestoreFrozenOperations {
  ...
  for(NSString *pendingOperationFile in pendingOperations) {
    ...
    JUSSNetworkOperation *pendingOperation = [NSKeyedUnarchiver unarchiveObjectWithFile:archivePath];
...
-(JUSSNetworkOperation*) operationWithURLString:(NSString*) urlString
  ...
  JUSSNetworkOperation *operation = [[self.customOperationSubclass alloc] initWithURLString:urlString params:body httpMethod:method];
...
-(NSData*) cachedDataForOperation:(JUSSNetworkOperation*) operation {
...
-(void) enqueueOperation:(JUSSNetworkOperation*) operation {
...
-(void) enqueueOperation:(JUSSNetworkOperation*) operation forceReload:(BOOL) forceReload {
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    [operation setCacheHandler:^(JUSSNetworkOperation* completedCacheableOperation) {
    ...
    dispatch_async(originalQueue, ^{
      ...
      if(index == NSNotFound) {
        ...
        JUSSNetworkOperation *queuedOperation = (JUSSNetworkOperation*) [_sharedNetworkQueue.operations objectAtIndex:index];
...
- (JUSSNetworkOperation*)imageAtURL:(NSURL *)url onCompletion:(MKNKImageBlock) imageFetchedBlock
{
  ...
  JUSSNetworkOperation *op = [self operationWithURLString:[url absoluteString]];
  
  [op 
   onCompletion:^(JUSSNetworkOperation *completedOperation)
   {
...
-(NSString*) cacheDirectoryName {
  ...
  NSString *cacheDirectoryName = [documentsDirectory stringByAppendingPathComponent:JUSSNetworkCACHE_DEFAULT_DIRECTORY];
  return cacheDirectoryName;
}

-(int) cacheMemoryCost {
  
  return JUSSNetworkCACHE_DEFAULT_COST;
}
