...
@class JUSSNetworkOperation;

typedef enum {
  JUSSNetworkOperationStateReady = 1,
  JUSSNetworkOperationStateExecuting = 2,
  JUSSNetworkOperationStateFinished = 3
} JUSSNetworkOperationState;

typedef void (^MKNKProgressBlock)(double progress);
typedef void (^MKNKResponseBlock)(JUSSNetworkOperation* completedOperation);
#if TARGET_OS_IPHONE
 ...
 *  
 *  @discussion
 *	You normally create an instance of this class using the methods exposed by JUSSNetworkEngine
 *  Created operations are enqueued into the shared queue on JUSSNetworkEngine
 *  JUSSNetworkOperation encapsulates both request and response
 *  Printing a JUSSNetworkOperation prints out a cURL command that can be copied and pasted directly on terminal
 *  Freezable operations are serialized when network connectivity is lost and performed when connection is restored
 */
@interface JUSSNetworkOperation : NSOperation {
 ...
 *  This property is readonly cannot be updated.
 *  Rather, updating this post dictionary doesn't have any effect on the JUSSNetworkOperation.
 ...
 *  JUSSNetworkKit doesn't freeze (readonly) GET operations even if they are marked as freezable
 */
 ...
 *	JUSSNetworkKit assumes a 7 day cache for images and 1 minute cache for all requests with no-cache set
 *	This property is internal to JUSSNetworkKit. Modifying this is not recommended and will result in unexpected behaviour
 */
 ...
 */
@property (copy, nonatomic) void (^operationStateChangedHandler)(JUSSNetworkOperationState newState);
 ...
 *  When set to YES, JUSSNetworkKit shows the NSError localizedDescription text as a notification when the app is in background and the network operation ended in error.
 ...
 *  When you enable caching on JUSSNetworkEngine, your completionHandler will be called with cached data first and then
...
// internal methods called by JUSSNetworkEngine only.
// Don't touch
-(BOOL) isCacheable;
